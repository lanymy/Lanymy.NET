...
- `GetFolderPath` 会把尾部使用的 `Path.AltDirectorySeparatorChar` 统一收敛到当前平台主目录分隔符，避免在 Windows 上形成 `dir/\` 这类混合结尾。
- 当输入只是“带扩展名、但不含父目录”的相对文件名（如 `README.txt`）时，`GetFolderPath` 现在返回空字符串，`InitDirectoryPath` 会随之保持 no-op，不再把该场景推进到不稳定的父目录解析。
- 对“真实存在、但没有扩展名”的文件路径（如 `README`），`GetFolderPath` / `GetFileName` / `GetPathType` 会优先按真实文件处理，而不是误判成目录。
...
- 对仅文件名且已明确是文件路径的输入（如 `README.txt`），`GetFolderPath` 返回空字符串而不是当前工作目录；上层若需要“相对于当前目录”的完整路径，应先自行做绝对化处理。
- 对“目录还不存在，但路径文本已经以目录分隔符结尾”的输入，`GetPathType(...)` 仍会按目录处理；这属于显式目录语法优先，而不是依赖文件系统探测。
