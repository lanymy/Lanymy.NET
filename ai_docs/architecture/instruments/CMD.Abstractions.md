...
- 这里的 `IsSuccess` 表示“命令执行流程未抛异常且退出码为 `0`”。
- 当前已显式记录进程退出码，调用方可以基于 `ExitCode` 做更严格的结果判断。
...
- 某些工具会把警告写入错误流，因此 `ErrorDataString` 有内容时也不一定代表命令失败，仍应优先结合 `ExitCode` 判断。
- 当前成功语义已经收紧到退出码级别，但仍没有超时、取消等更细粒度的执行控制。
...
- 若未来要增强自动化稳定性，建议继续补充超时、取消和参数安全拼接支持。
