...
namespace Lanymy.Common.AllTests
{
    ...
    public class LanymyIsolatedStorageTests
    {
        ...
        public void SaveStringTest()
        {
            ...
            Assert.IsNotNull(dencryptModel);
            Assert.AreEqual(model.SourceFileFullPath, dencryptModel.SourceFileFullPath);
            Assert.AreEqual(model.TargetFileFullPath, dencryptModel.TargetFileFullPath);

            dencryptModel = SecurityHelper.DecryptModelFromBase64String<ScheduleFileInfoModel>(encryptStr).SourceModel;
            Assert.IsNotNull(dencryptModel);
            Assert.AreEqual(model.SourceFileFullPath, dencryptModel.SourceFileFullPath);
            Assert.AreEqual(model.TargetFileFullPath, dencryptModel.TargetFileFullPath);
